...
import { IFavoritesComponent, PickStats } from '../../src/ports/favorites/types'
import { HTTPSuccessResponseBody } from '../../src/types'
...
describe('when getting the picks stats of some items', () => {
  ...
  beforeEach(() => {
    ...
    favoritesComponent = createFavoritesComponent({ fetch: fetchComponentMock }, favoritesURL)
  })
  ...
  describe("and the list of item ids doesn't exhaust the length of the signature server's URL", () => {
    ...
    beforeEach(async () => {
      ...
      response = {
        ok: true,
        data: itemIds.map((itemId, i) => ({ itemId, count: i, pickedByUser: i > 0 } as PickStats))
      }
  ...
  describe("and the list of item ids exhausts the length of the signature server's URL twice", () => {
    ...
    it('should have queried all the item ids', () => {
      expect(fetchMock.mock.calls.flatMap(call => new URL(call[0]).searchParams.getAll('itemId'))).toEqual(itemIds)
    })
  ...
  describe('and fetching the list of picks stats fails', () => {
    ...
    it('should throw an error with the message returned in the JSON message', () => {
      return expect(favoritesComponent.getPicksStatsOfItems(itemIds)).rejects.toThrowError(error)
    })
  })
})
