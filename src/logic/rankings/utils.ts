...
export function getUniqueItemsFromItemsDayData(itemDayDataFragments: ItemsDayDataFragment[], filters: RankingsFilters) {
  return itemDayDataFragments.reduce((acc, itemDayData) => {
    ...
    if (rankingItem) {
      ...
      rankingItem.volume = new BN(rankingItem.volume).add(new BN(itemDayData.volume)).toString()
    } else {
      acc[itemId] = {
        id: itemId,
...
export function getUniqueCreatorsFromCreatorsDayData(creatorsDayDataFragments: CreatorsDayDataFragment[]) {
  const accumulated = creatorsDayDataFragments.reduce((acc, creatorDayData) => {
    // if we're querying the total, we don't need to parse the itemId out of the fragment
    const creatorAddress = creatorDayData.id.indexOf('-') > -1 ? creatorDayData.id.split('-')[1] : creatorDayData.id

    const rankingCreator = acc[creatorAddress]
    if (rankingCreator) {
      if (Array.isArray(creatorDayData.uniqueCollectionsSales) && Array.isArray(rankingCreator.uniqueCollectionsSales)) {
        rankingCreator.uniqueCollectionsSales = [
          ...new Set([...rankingCreator.uniqueCollectionsSales, ...creatorDayData.uniqueCollectionsSales])
        ]
      } else {
        rankingCreator.uniqueCollectionsSales = creatorDayData.uniqueCollectionsSales
      }
      rankingCreator.sales += creatorDayData.sales
      rankingCreator.earned = new BN(rankingCreator.earned).add(new BN(creatorDayData.earned)).toString()
    } else {
  ...
  return Object.values(accumulated).reduce((acc, creatorDayData) => {
    ...
    acc[creatorsAddress] = {
      ...
      collections: Array.isArray(uniqueCollectionsSales) ? uniqueCollectionsSales.length : uniqueCollectionsSales
    }
    return acc
...
export function getUniqueCollectorsFromCollectorsDayData(
  ...
  const accumulated = collectorsDayDataFragments.reduce((acc, collectorDayData) => {
    // if we're querying the total, we don't need to parse the itemId out of the fragment
    const collectorAddress = collectorDayData.id.indexOf('-') > -1 ? collectorDayData.id.split('-')[1] : collectorDayData.id
    const rankingColllector = acc[collectorAddress]
    if (rankingColllector) {
      rankingColllector.purchases += collectorDayData.purchases
      rankingColllector.spent = new BN(rankingColllector.spent).add(new BN(collectorDayData.spent)).toString()
    } else {
