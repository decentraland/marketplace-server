...
export function createRentalsComponent(
  ...
  function buildGetRentalsParameters(filters: NFTFilters & Pick<RentalsListingsFilterBy, 'tenant' | 'status' | 'nftIds'>): string {
    ...
    parameters.category =
      filters.category === NFTCategory.PARCEL || filters.category === NFTCategory.ESTATE
  ...
  async function getRentalsListingsOfNFTs(nftIds: string[], status?: RentalStatus | RentalStatus[]): Promise<RentalListing[]> {
    const baseUrl = `${rentalsUrl}/v1/rentals-listings${buildGetRentalsParameters({
      rentalStatus: status
    })}`

    return queryMultipleTimesWhenExceedingUrlLimit(baseUrl, 'nftIds', nftIds, fetchComponent)
  }

  async function getRentalsListings(filters: NFTFilters): Promise<SignaturesServerPaginatedResponse<RentalListing[]>> {
    ...
    const parsedResult: SignaturesServerPaginatedResponse<RentalListing[]> = await response.json()

    if (!parsedResult.ok) {
      throw new ParseRentalListingResponseError(parsedResult)
    }

    return parsedResult
  }

  async function getRentalAssets(filters: GetRentalAssetFilters): Promise<RentalAsset[]> {
    function getSubgraphQueryOptions() {
      ...
      if (filters.contractAddresses && filters.contractAddresses.length > 0) {
        where.push(`contractAddress_in:[${filters.contractAddresses.map(contractAddress => `"${contractAddress}"`).join(',')}]`)
      }

      if (filters.tokenIds && filters.tokenIds.length > 0) {
        where.push(`tokenId_in:[${filters.tokenIds.map(tokenId => `"${tokenId}"`).join(',')}]`)
      }

      if (filters.lessors && filters.lessors.length > 0) {
        where.push(`lessor_in:[${filters.lessors.map(lessor => `"${lessor}"`).join(',')}]`)
      }
